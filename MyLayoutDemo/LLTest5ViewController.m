...
{
    /*
       这个例子主要用来介绍布局视图里面子视图的相对尺寸和相对边距间距的概念。对于线性布局来说子视图的相对尺寸我们需要用weight这个扩展属性来设置，而相对间距边距则通过将布局位置对象的值设置为大于0小于1的小数来确定。
     所谓子视图的相对值表示的是值不是一个明确的绝对值，而是一个比例或者比重值，最终的结果则会根据布局视图的剩余尺寸计算出来。布局视图的剩余尺寸是指将布局视图的尺寸扣除掉那些有绝对尺寸或者有绝对间距的子视图的尺寸和间距后剩余下来的空间。所有子视图的相对值的比重就是在布局视图的剩余尺寸下的比重。
        ...
        对于线性布局和框架布局来说当某个子视图的myXXXMargin的值大于0且小于1时表示使用的相对的间距或边距，否则使用的将是绝对的间距或边距。
