 ...
 */
@interface UIView(YSLayoutExt)


//视图四周的边界值,用于设置视图与其他视图或者与父布局视图在左，上，右，下，水平中心，垂直中心6个方位之间的关系
@property(nonatomic, readonly)  YSLayoutPos *leftPos;
@property(nonatomic, readonly)  YSLayoutPos *topPos;
@property(nonatomic, readonly)  YSLayoutPos *rightPos;
@property(nonatomic, readonly)  YSLayoutPos *bottomPos;
@property(nonatomic, readonly)  YSLayoutPos *centerXPos;
@property(nonatomic, readonly)  YSLayoutPos *centerYPos;


//视图宽度和高度的尺寸值,如果设置了尺寸值则以设置的尺寸值优先，否则以视图自身的frame中的高宽设置为准。
@property(nonatomic, readonly)  YSLayoutDime *widthDime;
@property(nonatomic, readonly)  YSLayoutDime *heightDime;


/*
 下面四个属性是上面leftPos,topPos,rightPos,bottomPos的equalTo设置NSNumber类型的值时的简化版本，表示距布局视图或者兄弟视图四周的具体边界值
 比如：
   v.ysLeftMargin = 10   <==>   v.leftPos.equalTo(@10)  表示左边偏移10
   v.ysRightMargin = 20  <==>   v.rightPos.equalTo(@20) 表示右边偏移20
 ...
 比如：
 v.ysLeftMargin = 10;  表示视图v离左边10个距离的位置
 v.ysLeftMargin = 0.1; 则表示视图v的左边距占用布局视图宽度的10%,如果布局视图宽度为200则左边距为20
...
@property(nonatomic, assign) CGFloat rightMargin  YSMETHODDEPRECATED("use ysRightMargin");
@property(nonatomic, assign) CGFloat bottomMargin YSMETHODDEPRECATED("use ysBottomMargin");

#endif

//因为上面四个属性值可能会和其他库的名字有冲突，所以这里增加前缀
@property(nonatomic, assign) CGFloat ysLeftMargin;
@property(nonatomic, assign) CGFloat ysTopMargin;
@property(nonatomic, assign) CGFloat ysRightMargin;
@property(nonatomic, assign) CGFloat ysBottomMargin;
...
/*
 ...
 v.ysCenterXOffset = 0  <==> v.centerXPos.equalTo(@0)   表示视图v在布局视图的水平居中
 v.ysCenterYOffset = 10 <==> v.centerYPos.equalTo(@10)  表示视图v在布局视图的垂直居中并往下偏移10个点
 v.ysCenterOffset = CGPointMake(10,10) <==> v.centerXPos.equalTo(@10);v.centerYPos.equalTo(@10)
 
 这三个属性的值最好别用于读取，而只是单纯用于设置
 */
@property(nonatomic, assign) CGFloat ysCenterXOffset;
@property(nonatomic, assign) CGFloat ysCenterYOffset;
...
@property(nonatomic, assign) CGPoint centerOffset  YSMETHODDEPRECATED("use ysCenterOffset");
...
/*
 ...
 v.ysWidth = 10    <=>   v.widthDime.equalTo(@10)
 
 这两个属性的值最好别用于读取，而只是单纯用于设置
 */
@property(nonatomic,assign) CGFloat ysWidth;
...
//清除所有视图的扩展属性的设置。
-(void)resetMyLayoutSetting  YSDEPRECATED("use resetYSLayoutSetting");
...
@interface YSBorderLineDraw : NSObject
...
typedef YSBorderLineDraw MyBorderLineDraw YSDEPRECATED("use YSBorderLineDraw");


/*布局视图基类，基类不支持实例化对象*/
@interface YSLayoutBase : UIView
...
/*
    ...
    contentLayout.ysLeftMargin = contentLayout.ysRightMargin = 0;
...
//指定布局四个要绘制的边界线。
@property(nonatomic, strong) YSBorderLineDraw *leftBorderLine;
@property(nonatomic, strong) YSBorderLineDraw *rightBorderLine;
@property(nonatomic, strong) YSBorderLineDraw *topBorderLine;
@property(nonatomic, strong) YSBorderLineDraw *bottomBorderLine;
//同时设置4个边界线。
@property(nonatomic, strong) YSBorderLineDraw *boundBorderLine;
...
//兼容老版本定义
typedef YSLayoutBase MyLayoutBase  YSDEPRECATED("use YSLayoutBase");
