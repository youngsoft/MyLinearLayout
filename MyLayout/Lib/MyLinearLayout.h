...
/**
 *设置子视图在垂直线性布局中的高度尺寸比重值和在水平线性布局下的宽度尺寸比重值。设置的范围是 0<=weight<=1。所谓比重指的是一个相对的尺寸值，视图的真实尺寸会根据布局视图的剩余尺寸乘以这个比重值所占的比例来得到真实的尺寸。假如一个垂直线性布局的高度为100，里面有A,B,C三个子视图。其中A视图的高度为20，B视图的比重值为0.4，C视图的比重值为0.6。那么最终视图A的高度是20，而视图B的高度就是(100 - 20)*(0.4/(0.4+0.6))=32，而视图C的高度就是(100 - 20)*(0.6/(0.4+0.6))=48。可以看到通过比重值的设置我们不需要明确的指定具体的尺寸，而是根据布局视图的尺寸大小来动态的决定自己的尺寸。使用比重时需要注意的几个点：
...
/**
 *线性布局，是一种里面的子视图将根据加入的顺序依次从上到下或者从左到右依次排列的布局。
 *从上到下依次排列的叫垂直线性布局，垂直线性布局里面的子视图在垂直方向的y轴上总是只有一个子视图存在；
 *从左到右依次排列的叫水平线性布局，水平线性布局里面的子视图在水平方向的x轴上总是只有一个子视图存在。
 */
...
/**
 *线性布局里面的所有子视图的整体停靠方向，所谓停靠是指布局视图里面的所有子视图整体在布局视图中的位置，系统默认的停靠是在布局视图的左上角。
 */
