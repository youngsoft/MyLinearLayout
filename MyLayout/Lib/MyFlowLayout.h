...
/**
 ...
 @note
   需要注意的是这里的方向和行的概念，行的概念它是相对于流式布局的方向而具体定义的。如果是垂直流式布局那么一行内的子视图将是水平排列，而整体则是每行从上往下垂直排列。
 而如果是水平流式布局那么一行内的子视图将是垂直排列，而整体则是每行从左往右水平排列。因此流式布局的方向所描述的是行整体的排列方向。

 @note
   所谓数量约束流式布局就是指事先约定每行的条目视图的数量，当一行的条目视图数量到达指定的数量后再添加条目视图时就会换行重新排列。而内容约束流式布局就是当添加到布局视图中的
 条目视图的宽度(垂直流式布局)之和或者高度(水平流式布局)之和超过布局视图的宽度或者高度时再添加条目视图就会换行进行重新排列。
...
/**
 初始化一个流式布局并指定布局的方向和每行条目视图的数量,如果数量为0则表示内容约束流式布局。
 
 @param orientation 布局的方向，这个方向是指的行与行之间排列方向，而不是行内的子视图的排列方向。
...
/**
 ...
 1. MyOrientation_Vert 表示行内子视图从左到右(RTL从右到左)水平排列，行与行之间依次从上到下垂直排列。这个方向是默认方向。
...
/**
 ...
 @note
 1. 垂直数量约束流式布局的高度设置为自适应时则以UIScrollView的尺寸作为一页展示的大小，因为指定了一页的子视图数量，以及指定了一行的子视图数量，因此默认也会自动计算出子视图的宽度和高度，而不需要单独指出高度和宽度(子视图的宽度你也可以自定义)，整体的分页滚动是从上到下滚动。(每页布局时从左到右再从上到下排列，新页往下滚动继续排列)：
 @code
     1  2  3
     4  5  6
     -------  ↓
 ...
 2. 垂直数量约束流式布局的宽度设置为自适应时则以UIScrollView的尺寸作为一页展示的大小，因为指定了一页的子视图数量，以及指定了一行的子视图数量，因此默认也会自动计算出子视图的宽度和高度，而不需要单独指出高度和宽度(子视图的高度你也可以自定义)，整体的分页滚动是从左到右滚动。(每页布局时从左到右再从上到下排列，新页往右滚动继续排列)
 @code
     1  2  3 | 7  8  9
     4  5  6 | 10 11 12
             →
 ...
 3. 水平数量约束流式布局的宽度设置为自适应时则以UIScrollView的尺寸作为一页展示的大小，因为指定了一页的子视图数量，以及指定了一行的子视图数量，因此默认也会自动计算出子视图的宽度和高度，而不需要单独指出高度和宽度(子视图的高度你也可以自定义)，整体的分页滚动是从左到右滚动。(每页布局时从上到下再从左到右排列，新页往右滚动继续排列)
 @code
     1  3  5 | 7  9   11
     2  4  6 | 8  10  12
             →
 ...
 4. 水平数量约束流式布局的高度设置为自适应时则以UIScrollView的尺寸作为一页展示的大小，因为指定了一页的子视图数量，以及指定了一行的子视图数量，因此默认也会自动计算出子视图的宽度和高度，而不需要单独指出高度和宽度(子视图的宽度你也可以自定义)，整体的分页滚动是从上到下滚动。(每页布局时从上到下再从左到右排列，新页往下滚动继续排列)
  @code
     1  3  5
     2  4  6
    --------- ↓
...
/**
 ...
 1. 如果是垂直流式布局则表示每行内子视图的上中下对齐方式，这里的对齐基础是以每行中的最高的子视图为基准。这个属性只支持：
   @code
    MyGravity_Vert_Top     顶部对齐
    MyGravity_Vert_Center  垂直居中对齐
    MyGravity_Vert_Bottom  底部对齐
    MyGravity_Vert_Fill    两端对齐
    MyGravity_Vert_Stretch 如果子视图未设置约束则子视图高度被拉伸
  ...
  @endcode
 2. 如果是水平流式布局则表示每行内子视图的左中右对齐方式，这里的对齐基础是以每行中的最宽的子视图为基准。这个属性只支持：
    @code
     MyGravity_Horz_Left    左边对齐
     MyGravity_Horz_Center  水平居中对齐
     MyGravity_Horz_Right   右边对齐
     MyGravity_Horz_Fill    两端对齐
     MyGravity_Horz_Stretch 如果子视图未设置约束则子视图宽度被拉伸
