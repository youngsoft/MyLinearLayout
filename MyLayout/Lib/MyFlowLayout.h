...
/**
 流式布局是一种条目视图(item)按照添加的顺序依次排列，当满足某种规则后会另起一排再重新排列，最终呈现为多行多列展示形态的一种布局。
 这里排列的整体分为垂直和水平两个方向，换排规则主要有数量限制和尺寸限制两种。因此流式布局一共分为垂直数量限制、垂直尺寸限制、水平数量限制、水平尺寸限制四种布局。流式布局主要应用于那些有规律排列的场景，在某种程度上可以作为UICollectionView的替代品，同时流式布局实现了CSS3的flexbox的几乎全部功能。
 
 @note
   需要注意的是这里的方向和排的概念，排的概念是相对于流式布局的方向而具体定义的：如果是垂直流式布局那么一排内的条目视图将按从左往右水平排列，而每排则从上往下垂直排列；如果是水平流式布局那么一排内的条目视图将按从上往下垂直排列，而每排则从左往右水平排列。流式布局所指的方向是排之间排列的方向，而不是指每排内条目视图排列的方向。

 @note
   所谓数量限制流式布局是指约定每排内条目视图的数量，当一排的条目视图数量超过约定的数量后，如果再添加新的条目视图就会换按约定的方向新起一排重新排列；而尺寸限制流式布局则约定一排内添加到布局视图中的所有条目视图的宽度(垂直流式布局)之和或者高度(水平流式布局)之和超过布局视图的宽度或者高度时，如果再添加新的条目视图就会按约定的方向新起一排重新排列。
...
/**
 ...
 @param orientation 布局的方向，这个方向是指的排与排之间排列方向，而不是排内条目视图的排列方向。
 @param arrangedCount 每排内的条目视图的数量，如果每排内的条目视图的数量不固定则设置为0表示为尺寸限制流式布局。
 @return 返回流式布局对象实例。
 */
...
/**
 ...
 1. MyOrientation_Vert 表示一排内条目视图从左往右(RTL从右往左)水平排列，排与排之间依次从上往下垂直排列。这个方向是默认方向。
 
 2. MyOrientation_Horz 表示一排内条目视图从上往下垂直排列，排与排之间依次从左往右(RTL从右到左)水平排列。
 */
@property (nonatomic, assign) MyOrientation orientation;

/**
 每排内的条目视图数量。默认是0表示每排内的条目视图的数量不固定，而是根据一排内条目视图的尺寸之和是否超过布局视图尺寸自动换排处理。如果非0则每排内的条目视图数量大于这个值后会自动换排处理。
 */
@property (nonatomic, assign) NSInteger arrangedCount;

/**
 为流式布局提供分页展示的能力,默认是0表不支持分页展示。当设置为非0时则要求必须是arrangedCount的整数倍数，表示每页的子视图的数量。而arrangedCount则表示每排内的子视图的数量。当启用pagedCount时如果流式布局的父视图是UIScrollView或者其派生类就会有分页滚动的效果(要想按页滚动，需要将UIScrollView的 pagingEnabled属性设置为YES)。只有数量限制流式布局才支持分页展示的功能，通过pagedCount和将布局视图的高度或者宽度设置为自适应尺寸进行配合就能实现不同的分页展示能力:
 @note
...
/**
 ...
 因此当此属性设置为YES后，所有条目视图的排列顺序将可能不会按照添加顺序进行排列了。
 
 @note
 如果在尺寸限制流式布局中使用此属性时，请在将所有子视图添加完毕并且初始布局完成后再设置这个属性，否则如果预先设置这个属性则后续每添加一个条目视图都会触发重新布局，从而引起性能问题。
 */
...
/**
  布局完成后的总行数，这个属性只有在布局完成后或者调用sizeThatFits方法评估后 读取到的值才是正确的。你也可以通过kvo的方式来读取布局后的行数,或者在endLayoutBlock中读取此属性值。
 */
@property (nonatomic, assign, readonly) NSInteger arrangedLines;
